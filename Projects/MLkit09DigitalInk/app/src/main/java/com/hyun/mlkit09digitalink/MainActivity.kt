package com.hyun.mlkit09digitalink
...
import com.hyun.mlkit09digitalink.databinding.ActivityMainBinding
...
//
    ...
    private fun clickRecognition(){
        //Digital Ink Recognizer 객체를 가져와서 .. MyDrawingView에 써 있는 손글씨 데이터를 읽어오기..

        //1. Digital Ink Recognizer 객체를 생성하는 3단계
        // 언어별 모델 식별자 준비
        val modelIdentifier:DigitalInkRecognitionModelIdentifier?= DigitalInkRecognitionModelIdentifier.fromLanguageTag("ko")// 가이드 문서에 있는 나라 코드
        ...
        remoteModelManager.isModelDownloaded(model).addOnSuccessListener {
            ...
            if (it){
                ...
                //2. 인식할 Digital Ink 준비.. - MyDrawingView에서 그려진 좌표값을 저장한 Ink 객체를 가져와야함
                ...
                remoteModelManager.download(model, DownloadConditions.Builder().build()).addOnSuccessListener {
                    binding.tv.append("한국어 인식모델 디운로드 완료!!")
                }
            }
        }
