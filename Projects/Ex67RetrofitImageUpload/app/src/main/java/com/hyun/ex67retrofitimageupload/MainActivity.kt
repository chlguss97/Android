package com.hyun.ex67retrofitimageupload
...
import com.hyun.ex67retrofitimageupload.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private val resultLauncher= registerForActivityResult(ActivityResultContracts.StartActivityForResult()){
        ...
        if(uri != null){
            ...
            //Retrofit을 이용한 Http 통신을 할때..
            //파일을 업로드 하려면.. 파일의 실제 경로가 필요함
            //uri는 콘텐츠 경로임. -- 그래서 Uri에 해당하는 파일의 실제 경로를 구해와야함.
            //uri --> 절대주소(실제 물리적인 저장소 위치)로 변환
    ...
    private fun getRealPathFromUri(uri:Uri): String? {
        ...
        while(true){
            ...
            val len:Int =inputStream.read(buf) // 스트림을 통해 읽어들인 바이트들을 buf 배열에 넣어줌. -- 읽어들인 바이트 수를 리턴해줌
            if(len<=0 ) break
            outputStream.write(buf,0,len)  //off: 편차= 읽고나서 편차 주는것
        }//while
    ...
    private fun clickUpload(){
        ...
        val part: MultipartBody.Part= MultipartBody.Part.createFormData("img1",file.name,requestBody) // 식별자, 파일이름, 요청Body [택배 스트로폼상자]
