...
import com.hyun.tpsearchplacebykakao.databinding.ActivitySignupBinding
...
class SignupActivity : AppCompatActivity() {
    ...
    private fun clickSignup(){
        ...
        // Firebae Firestore DB에 저장하기 - 프로젝트 연동부터
        ...
        userRef.whereEqualTo("email",email).get().addOnSuccessListener {
            //혹시 같은 emial값을 가진 도큐먼트가 여러개 일수도 있어서..
            if (it.documents.size>0){ // 개수가 0개 이상이면.. 같은 이메일이 있다는 것임.
                AlertDialog.Builder(this).setMessage("중복된 이메일이 있습니다. 다시 확인해주세요").create().show()
                binding.inputLayoutEmail.editText!!.requestFocus()
                binding.inputLayoutEmail.editText!!.selectAll()
            }else{//중복된게 이메일이 없다면 저장.
