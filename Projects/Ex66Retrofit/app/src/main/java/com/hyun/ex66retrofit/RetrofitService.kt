...
interface RetrofitService {
    ...
    //1. 단순하게 GET방식으로 json문자열을 읽어오는 코드 만들어줘.. [작성해준 코드를 가진 객체를 리턴해줌]
    ...
    //4. GET방식으로 값을 한방에 Map collection으로 전달하기
    @GET("/04Retrofit/aaa.php")
    fun getMethodTest2(@QueryMap data:Map<String, String> ) : Call<BoardItem>

    //5. POST방식으로 값 전달 - 방법1. 객체를 통으로 전달..하면 자동으로 json문장ㄹ로 변환하여 서버로 전송함
    ...
    //7. GET방식으로 json array 값을 받아와서 곧바로 List,BoardItem>으로 응답해주는 코드 만들어줘..
