package com.hyun.mlkit06documentscanner
...
import com.hyun.mlkit06documentscanner.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    val scannerLauncher= registerForActivityResult(ActivityResultContracts.StartIntentSenderForResult()){
        if(it.resultCode== RESULT_OK){
            ...
            val request: GmsDocumentScanningResult? = GmsDocumentScanningResult.fromActivityResultIntent(it.data)

            binding.tv.text=""

            //1) JPEG 이미지로 스캔할때.. 이미지의 경로 uri 받기
