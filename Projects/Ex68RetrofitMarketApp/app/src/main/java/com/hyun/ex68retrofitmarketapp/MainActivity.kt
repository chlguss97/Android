...
import com.hyun.ex68retrofitmarketapp.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    //액티비티가 화면에 완전히 보여질 때 자동 호출되는 라이프 사이클 메소드
    ...
    private fun loadDB(){
        ...
        retrofitService.loadDataFromServer().enqueue(object : Callback<List<MarketItem>>{
            override fun onResponse(call: Call<List<MarketItem>>, response: Response<List<MarketItem>>) {
                ...
                //결과로 받아온 List<MarketItem> 를 리사이클로ㅓ뷰에 보여주기
                val items: List<MarketItem>? =response.body()
                items?.forEach{
                    itemList.add(0,it) //추가되는 아이템을 가장 앞에 배치..
